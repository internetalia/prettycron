//////////
// This is a modified version of this library - as i didnt like its output - you should still npm installit to get its dependencies but this is what is used in the code
/////////
...
if ((!moment || !later) && (typeof require !== 'undefined')) {
  ...
  (function() {
     ...
     */
    var scheduleToSentence = function(schedule,workingDay) {
        ...
        if(schedule['h']) { // runs only at specific hours
          if (schedule['m']) { // and only at specific minutes
            if (schedule['m'].length == 1 && schedule['m'][0] == 0) {
                output_text += numberList(schedule['h']) + ' hour, on the hour';
              }
              else{
      ...
      if (schedule['D']) { // runs only on specific day(s) of month
        output_text += ' on the ' + numberList(schedule['D']) + (workingDay ? ' working day' : '');
        if (!schedule['M']) {
     ...
     */
    var toString = function(cronspec, sixth, workingDay) {
      var schedule = later.parse.cron(cronspec, sixth);
      return scheduleToSentence(schedule['schedules'][0],workingDay);
    };
